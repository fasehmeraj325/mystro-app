...
3. Create a `.env` file in this folder with:
   ...
   `DATABASE_URL` is your Postgres connection string. `SESSION_SECRET` signs login sessions — any long random string (`node -e "console.log(require('crypto').randomBytes(32).toString('hex'))"` generates one). The `SMTP_*`/`APP_URL` vars power emailing clients their application link (see [Emailing clients](#emailing-clients) below) — leave them out if you don't need that yet. `.env` is already in `.gitignore` — never commit it.
...
- The structured form data is stored as JSON in Postgres (the `client_info` column), alongside plain columns for name/email/phone/status/company_id used by the dashboard table. Uploaded files are saved under `data/uploads/<submission-id>/` on disk, with their metadata (an array per document field, supporting multiple files) stored in Postgres.
...
- Set `DATABASE_URL`, `SESSION_SECRET`, and the `SMTP_*`/`APP_URL` vars in the host's environment variable settings (not in the code).
- Submission, company, and user data all live in Postgres, so they survive redeploys regardless of the host's disk. Uploaded **documents** are still saved to local disk under `data/uploads/`, so the host needs a **persistent disk/volume** mounted there — otherwise document files (though not the database records) are lost on restart/redeploy.
