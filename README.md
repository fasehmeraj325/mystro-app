...
- **White-label branding** (`/settings.html`): a company can set its own logo, business name, sender name, and a brand color. The client-facing pages (`apply-form.html`, `apply-upload.html`) read this via the public `/api/public/companies/:slug` endpoint and swap in the logo in place of the Docklio mark, and set the color as the `--brand` CSS variable that the rest of the page's buttons/progress-bars/accents already key off — no per-component styling needed. The brand mark on those two pages is deliberately not a link back to Docklio's own site (unlike every other page), since a client filling out someone else's branded form shouldn't be one click away from a third party's homepage. The logo itself is a public Cloudinary asset (not `authenticated`, unlike client documents) keyed by company id with `overwrite: true`, so re-uploading replaces it in place.
