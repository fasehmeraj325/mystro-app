...
- The dashboard at `/dashboard.html` shows every one of the logged-in company's submissions — including in-progress drafts, tagged **In Progress** — with quick stats (including an "In progress" count), a "Send application link to a client" box (also shows the direct `/apply/:slug` link with a copy button), a client-name search box, and a table with a compact Form%/Docs% progress column, where clicking a row opens the full detail: the same two progress bars, all the fact-find sections, a **Download Client Form (PDF)** button, an **Export documents (ZIP)** button for that one client's files, individual document blocks (each with its own heading and download link), and a status control (New / In Review / Approved / Rejected). An **Export all documents** button at the top of the dashboard downloads every client's documents in one ZIP, one folder per client — no OneDrive/Drive API integration needed, just point your browser's download folder (or wherever you extract it) at your cloud-synced folder and it lands there like any other file.
- **Document sanity-check** (optional, needs `GEMINI_API_KEY`): each uploaded document is checked by Gemini (`checkDocumentQuality()` in `server.js`) for whether it looks like the right document type, is legible, and — for ID documents — whether it looks expired. Runs synchronously during the upload request (adds a couple seconds), never blocks the upload on failure, and is skipped entirely (no-op, no error) if the API key isn't set. A flagged file shows an inline warning to the client on the upload page and to the company in the dashboard's document list.
- **New-application notifications**: when a client finishes (`Draft` → `New`), every active user at that company gets an email with the client's name/email, a link straight to that application (`/dashboard.html?open=<id>`, which auto-opens the detail modal), and a summary of any document sanity-check flags — sent best-effort, after the client's own request already succeeded, so a mail failure never blocks their submission.
- **White-label branding** (`/settings.html`): a company can set its own logo, business name, sender name, a brand color, and a light/dark theme for the client-facing pages. The client-facing pages (`apply-form.html`, `apply-upload.html`) read this via the public `/api/public/companies/:slug` endpoint and swap in the logo in place of the Docklio mark, set the color as the `--brand` CSS variable that the rest of the page's buttons/progress-bars/accents already key off, and (if `theme === "light"`) set `data-theme="light"` on `<html>`, which `style.css` uses to override the `:root` variables (`--bg`, `--page-text`, `--pill-bg`, etc.) for a light-mode reading of the same components — no per-component styling needed either way. The brand mark on those two pages is deliberately not a link back to Docklio's own site (unlike every other page), since a client filling out someone else's branded form shouldn't be one click away from a third party's homepage. The theme choice only affects the client-facing pages — the company's own dashboard always stays dark. The logo itself is a public Cloudinary asset (not `authenticated`, unlike client documents) keyed by company id with `overwrite: true`, so re-uploading replaces it in place.
- **Document/form customization** (`/settings.html`): a company can turn off individual document types they don't need (e.g. skip Company Tax Return if they never work with self-employed clients) and turn off entire optional fact-find sections (Additional Income, Real Estate Assets, Assets, Liabilities, Ongoing Expenses). Driver's Licence/Passport and the core identity/address/employment sections can never be turned off — some form of ID and the basics are always required. Stored as `documentConfig`/`formConfig` on the company (an object listing only the *off* items — everything absent defaults to on, so existing companies are unaffected). `progress.js`'s `docApplicability`/`computeFormProgress` take these as an optional second argument and are the single place both the client pages and the dashboard/PDF filter by them, so a disabled item disappears consistently everywhere: the client never sees the field, `buildClientInfo()` omits the whole key from what's submitted (not just hidden via CSS), the progress percentages exclude it from their denominator, the dashboard's detail view and the PDF export skip the section entirely (since there's no data for it to render), and the upload endpoint itself rejects a direct POST to a disabled document type as defense in depth.
...
- **No password-change UI** — `/settings.html` covers branding (business name, sender name, logo, brand color), but changing your own password still requires direct database access.
